...
public sealed class MapLogger : IDisposable
{
    ...
    public MapLogger(string logFolderPath, string logNameWithoutExt, int maxSizeInKB, int maxNumberOfFiles) {
        ...
        List<int> indices = MapLogger.LookForFileIndices(files);
    ...
    private unsafe void WriteRecord(string message,
        ...
        int recordByteLength = MapLogger.ComputeRecordSize(messageLength + callerFilePathLength + lineDigitsCount);
    ...
    private static List<int> LookForFileIndices(string[] logFiles) {
        ...
        for (int i = 0; i < logFiles.Length; ++i) {
            ...
            System.Diagnostics.Debug.Assert(filePath.Length > 4, message: "too small for a log file");
            int indexEndDot = filePath.Length - MapLogger._extension.Length;
            int indexStartDot = filePath.LastIndexOf(value: '.', indexEndDot - 1) + 1;
            System.Diagnostics.Debug.Assert(indexStartDot != 0, message: "logFiles should contains only log files.");
    ...
    private static string GetLogFilePath(string filePath, int index) {
        return filePath + index + MapLogger._extension;
    }
    ...
    private unsafe void MapFile(long capacity) {
        ...
        if (_logFilePathes.Count > _maxNumberOfFiles) {
            ...
            string fileToDel = MapLogger.GetLogFilePath(_logFileBasePath, toDeleteIdx);
            if (File.Exists(fileToDel)) {
                File.Delete(fileToDel);
            }
        }

        string filePath = MapLogger.GetLogFilePath(_logFileBasePath, _currentFileIndex);
        if (!File.Exists(filePath)) {
            ...
            if (_headOffset >= capacity) {
                ...
                _headOffset = 0L;
                _currentFileIndex += 1;
                filePath = MapLogger.GetLogFilePath(_logFileBasePath, _currentFileIndex);
                File.Create(filePath!).Dispose();
            }
        }

        _mappedFile = MemoryMappedFile.CreateFromFile(filePath, FileMode.Open, mapName: null, capacity
                                                      , MemoryMappedFileAccess.ReadWrite);
        _mapView = _mappedFile.CreateViewAccessor(offset: 0, capacity);
    ...
    private unsafe void ReleaseMap(string filePath, int indexFile) {
        ...
        if (_mappedFile != null) {
            ...
            FileStream file = File.OpenWrite(MapLogger.GetLogFilePath(filePath, indexFile));
...
#pragma warning disable CS1573 // parameters sourceFile and lineNumber should not have a documentation
    ...
    public void Debug(string message, [CallerFilePath] string sourceFile = "", [CallerLineNumber] int lineNumber = -1) {
        ...
        if (MinLevel <= Level.Debug) {
            WriteRecord(message, MapLogger._levelStringDebug, Path.GetFileName(sourceFile.AsSpan()), lineNumber);
        }
#endif
    }
    ...
    public void Info(string message, [CallerFilePath] string sourceFile = "", [CallerLineNumber] int lineNumber = -1) {
        ...
        if (MinLevel <= Level.Information) {
            WriteRecord(message, MapLogger._levelStringInfo, Path.GetFileName(sourceFile.AsSpan()), lineNumber);
        }
#endif
    }
    ...
    public void Warning(string message, [CallerFilePath] string sourceFile = "", [CallerLineNumber] int lineNumber = -1
        ...
        if (MinLevel <= Level.Warning) {
            WriteRecord(message, MapLogger._levelStringWarn, Path.GetFileName(sourceFile.AsSpan()), lineNumber);
        }
#endif
    }
    ...
    public void Error(string message, [CallerFilePath] string sourceFile = "", [CallerLineNumber] int lineNumber = -1) {
        ...
        if (MinLevel <= Level.Error) {
            WriteRecord(message, MapLogger._levelStringError, Path.GetFileName(sourceFile.AsSpan()), lineNumber);
        }
#endif
    }
